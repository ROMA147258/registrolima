...
export function RegistrationView({ onShowLogin, onRegisteredSuccess }) {
  ...
  const validateForm = () => {
    ...
    if (!isCoordinadorDistrital) {
      if (!formData.local_asignado || formData.local_asignado.trim() === '' || formData.local_asignado === 'No aplica') {
        errors.local_asignado = isCoordinadorLocal
          ? 'Seleccione el local de votación a su cargo.'
          : 'Seleccione el colegio o local de votación asignado.';
      }
    }
      ...
      <div style={{
        ...
        <form onSubmit={handlePreSubmit} noValidate style={{ marginTop: '20px', display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <div style={{ border: (fieldErrors.rol_electoral || fieldErrors.distrito_asignado || fieldErrors.local_asignado) ? '2px solid #ef4444' : '1.5px solid #bae6fd', borderRadius: '14px', padding: '16px', background: '#fafbfc', transition: 'border 0.2s ease' }}>
            ...
            {/* Asignación Territorial: Si es Coordinador Distrital solo Distrito, de lo contrario Distrito y Local Asignado */}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                ...
                <div>
                  ...
                  <CustomSearchableSelect
                    ...
                    onChange={handleChange}
                    options={DISTRITOS_LIMA}
                    placeholder="Seleccione Distrito Asignado"
                    icon={MapPin}
                    required
                    hasError={!!fieldErrors.distrito_asignado}
                    errorMsg={fieldErrors.distrito_asignado}
                ...
                {/* Asignar Local / Local Asignado */}
                <div>
                  <label className="form-label" style={{ color: !formData.distrito_asignado ? '#94a3b8' : '#1e293b', fontSize: '0.8rem', fontWeight: 700 }}>
                    {isCoordinadorLocal ? 'Asignar Local (Centro de Votación)' : 'Asignar Local (Colegio Asignado)'} <span style={{ color: '#ef4444' }}>*</span>
                  </label>
                  <CustomSearchableSelect
                    ...
                    placeholder={
                      !formData.distrito_asignado
                        ? "Primero seleccione Distrito Asignado"
                        : (formData.distrito_asignado ? `Seleccione Local en ${formData.distrito_asignado}` : "Seleccione Local Asignado")
                    }
                    icon={School}
                    required
